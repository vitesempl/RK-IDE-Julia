...
export ide_solve, ide_delay_solve

function ide_solve(idefun, Core, delays_int, history, tspan, step)
    ...
    for ij = 1:nint
        step    = trunc(Int,(t0 - tj[ij])/htry); # The number of memorized 
    ...
    K[:,1,k] = ones(neq).*idefun(t[k], y[:,1], F);
    ...
    while t[k] < tf
        ...
        for i = 2 : s
            ...
            # K2-S
            K[:, i, k] = ones(neq).*idefun(ti, Y[:,i], F+Z[:,i]);
        end
        #==========================================================#
        ...
        K[:,1,k+1] = ones(neq).*idefun(t[k+1], y[:,k+1], F);
...
#==============================================================================================#
#==============================================================================================#
function ide_delay_solve(idefun,delays,Core,delays_int,history,tspan,step)
